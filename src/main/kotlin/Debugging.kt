 ...
 *
 * Точки останова на строке полезны, но есть несколько других мест, где может быть полезно остановить выполнение:
 *
 * - Когда вызывается основной конструктор.
 *
 * - Когда вызывается определённый метод. В том числе тот, который был переопределён, или тот, у которого исходный код неверен или отсутствует.
 *
 * - Когда генерируется исключение определённого типа.
 *
 * - Следующая строка текущего метода, когда выполнение программы уже остановлено.
 *
 * - Первая строка метода, который будет вызван и добавлен в стек вызовов следующим.
 *
 * - Следующая строка вызывающего метода после текущего выполняющегося метода извлекается из стека вызовов.
 *
 *
 * Точки останова для конструктора и методов
 *
 * Чтобы установить точку останова метода, щелкните область напротив объявления метода.
 *
 * Выполнение остановится на первой строке метода.
 *
 * Первичный конструктор класса может не содержать никаких операторов и даже не иметь объявлений свойств или круглых скобок.
 *
 * Выполнение остановится прямо внутри конструктора, который в байт-коде называется <init>.
 *
 * Используя этот инструмент, вы можете отслеживать вызовы конструктора, даже если имеете дело с классами, имеющими неявные объявления конструктора.
 *
 *
 * Точки останова для исключений
 *
 * Когда происходит сбой приложения, вы можете увидеть его трассировку стека, но этого не всегда достаточно. Просмотр кадров стека и наблюдение за состоянием приложения непосредственно перед сбоем может оказаться очень полезным.
 *
 * Нажатие "Run → View Breakpoints" позволяет увидеть все текущие точки останова и установить точки останова для исключений.
 *
 * Kotlin использует тот же механизм исключений, что и Java, поэтому выберите параметр «Java Exception Breakpoints». Сделав это, вы сможете выбрать любой из типов исключений, доступных в текущем проекте, и настроить любые необходимые детали.
 *
 * Например, вы можете выбрать остановку на перехваченных исключениях, неперехваченных исключениях или на обоих.
 *
 * Теперь выполнение будет остановлено при возникновении исключения, отвечающего указанным условиям.
 *
 *
 * Степпинг
 *
 * Итак, у вас программа остановлена на точке останова. Но что произойдет, когда будет выполнена следующая строка? Или несколькими строками позже? Какое переопределение метода будет вызвано? Что произойдет после возврата текущего метода?
 *
 * - Step Over возобновит выполнение до следующей строки, не переходя ни к одному из методов, вызванных из текущего. Это может быть полезно, если вы хотите проверить выполнение кода построчно.
 *
 * - Step Into перейдёт к следующему методу, вызываемому текущей строкой, или просто перейдёт, если его нет.
 *
 * - Force Step Into (который имеет тот же значок, что и Step Into, но с красной стрелкой вместо синей). Этот инструмент позволяет легко перейти к библиотечному коду, а не оставаться на уровне собственного кода. Force Step Into будет работать даже без исходного кода, а также для классов, созданных во время выполнения, без доступного байт-кода. В этом случае вы не увидите выполняемый код, но кадры стека всё равно будут видны, что даст вам представление о том, что делает этот метод.
 *
 * - Step Out приостановит выполнение после завершения текущего метода и вернёт вас в контекст вызывающего метода.
 *
 * - Кроме того, вы можете использовать «Run to Cursor». Если навести курсор на номер строки, она будет выделена. И если вы нажмете на неё, программа возобновит работу до тех пор, пока не достигнет выбранной строки, а затем снова приостановится. Это значит, что вместо добавления точки останова и возобновления можно просто щелкнуть нужную строку.
 */
fun debugging() {
...
/**
 * Узнайте, какие подтипы списка используются при его создании с помощью функции listOf() Kotlin с нулем, одним или несколькими элементами.
 *
 * Напишите эти имена типов через запятую в поле ответа.
 */
fun listDebug() {
    val l1 = listOf<Int>() // EmptyList
    val l2 = listOf(4) // SingletonList
    val l3 = listOf(5, 6) // ArrayList
}
