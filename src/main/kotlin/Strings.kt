...
const val N = 99999999 // Количество итераций теста

/**
 * StringBuilder — это мощный инструмент, предоставляющий изменяемые строки. Однако это может быть не лучшим решением в ситуациях, когда нескольким потокам необходимо одновременно изменить один и тот же объект StringBuilder.
 *
 * Вот тут-то и появляется StringBuffer. Это одноранговый класс StringBuilder с дополнительной функцией: синхронизацией (механизм, который гарантирует, что только один поток может получить доступ к общему ресурсу одновременно, предотвращая условия гонки и другие проблемы параллелизма). Это делает StringBuffer потокобезопасным и лучшим выбором для многопоточных сред. Однако существует компромисс между синхронизацией и производительностью.
 *
 * Как и в случае со StringBuilder, у нас есть различные способы создания объекта StringBuffer. Мы можем инициализировать его как пустой объект, установить для него подходящее начальное содержимое (String или CharSequence) или указать начальную ёмкость.
 *
 * Стоит отметить, что StringBuffer — это класс Java. Однако вы можете использовать его непосредственно в Kotlin, ничего не импортируя, поскольку он принадлежит пакету java.lang, который импортируется автоматически.
 *
 * StringBuffer имеет много общих с StringBuilder методов. Однако важно отметить, что под капотом есть решающая разница. Более пристальный взгляд на класс StringBuffer внутри библиотеки java.lang показывает, что все методы StringBuffer отмечены модификатором Synchronized. Именно это делает StringBuffer синхронизированным и потокобезопасным, позволяя использовать его нескольким потокам без необходимости внешней синхронизации.
 *
 * - capacity - Получает текущую ёмкость (количество символов, которые можно сохранить без перераспределения) StringBuffer.
 *
 * - length - Получает текущую длину (количество символов) StringBuffer.
 *
 * - append - Добавляет строку в конец StringBuffer
 *
 * - insert - Вставляет строку в определённую позицию в StringBuffer
 *
 * - delete - Удаляет диапазон символов из StringBuffer
 *
 * - replace - Заменяет диапазон символов новой строкой
 *
 * - reverse - Меняет порядок символов в StringBuffer
 *
 * При использовании StringBuffer для добавления или вставки данных из исходной последовательности, которая используется несколькими потоками, важно убедиться, что исходная последовательность не изменяется другими потоками во время выполнения операции.
 *
 * Это связано с тем, что синхронизируются только методы StringBuffer, а не сам StringBuffer.
 *
 * Лучшие сценарии использования StringBuilder:
 *
 * - Конкатенация строк (String concatenation). Если вы работаете над задачами, включающими тяжелые операции конкатенации строк, например создание динамических запросов SQL или создание больших полезных данных JSON/XML, StringBuilder — отличный выбор. Его изменчивая природа позволяет эффективно добавлять строки, уменьшая потребление памяти и избегая создания ненужных объектов.
 *
 * - Ведение журнала и форматирование (Logging and formatting). При регистрации или форматировании сообщений, содержащих динамические или переменные данные, StringBuilder оказывается полезным. Добавляя различные значения или переменные в StringBuilder, вы можете эффективно создавать сложные сообщения журнала или форматированные строки без ненужных затрат.
 *
 * - Генерация разметки HTML или XML (Generating HTML or XML markup). При динамическом создании разметки HTML или XML подходящим выбором может быть StringBuilder. Он позволяет эффективно создавать структуру и содержимое разметки путём добавления тегов, атрибутов и значений без ненужных накладных расходов.
 *
 *
 * Лучшие сценарии использования StringBuffer:
 *
 * - Параллельная обработка данных (Concurrent data processing). При одновременной обработке данных в многопоточной среде StringBuffer можно использовать для безопасного управления общими строками. Например, если нескольким потокам необходимо извлечь и обработать данные из общего файла журнала или потока данных, можно использовать StringBuffer, чтобы гарантировать согласованность и точность обработки данных.
 *
 * - Многопоточная генерация текста (Multi-threaded text generation). В приложениях, генерирующих текст одновременно, например при создании отчётов или форматировании документов, StringBuffer можно использовать для безопасного построения окончательного текстового вывода. Используя StringBuffer, несколько потоков могут добавлять текст в общий буфер, не беспокоясь о проблемах синхронизации.
 *
 * - Потокобезопасное кэширование строк (Thread-safe string caching). В приложениях, которые кэшируют часто используемые строки, таких как веб-серверы или системы баз данных, StringBuffer можно использовать для реализации поточно-безопасного кэша. Используя StringBuffer, несколько потоков могут безопасно получать доступ к кэшированным строкам и манипулировать ими без повреждения или несогласованности данных.
 *
 * Преобразование StringBuffer в String позволяет получить неизменяемое представление измененного содержимого для дальнейшей обработки или передачи его методам, ожидающим параметр String. Этого можно добиться с помощью метода toString.
 *
 * Метод toString возвращает новый объект String, представляющий содержимое StringBuffer. Таким образом, любые последующие изменения, внесенные в StringBuffer после вызова toString, не повлияют на объект String. Важно преобразовать StringBuffer в String, когда вам больше не нужно изменять содержимое или вы хотите обеспечить его неизменяемость.
 *
 * Пытаясь выбрать между StringBuilder и StringBuffer, вам следует учитывать компромисс между синхронизацией и производительностью. Синхронизация в StringBuffer обходится дорого: она вносит небольшие накладные расходы, что делает ее немного медленнее, чем StringBuilder. Поэтому, если безопасность потоков не является проблемой в вашем конкретном сценарии, будет более эффективно использовать StringBuilder. Стоит отметить, что StringBuffer был частью Java с момента его первоначального выпуска (Java 1.0), а StringBuilder был представлен как облегченная альтернатива в Java 5.0, предлагающая улучшенную производительность для непотокобезопасных операций.
 */
fun stringBuffer() {
    // Пустой StringBuffer с начальной ёмкостью по умолчанию (16 байт)
    ...
    println(buffer1.toString()) // "Hyperskill"

    // StringBuffer с некоторым исходным содержимым
    val buffer2 = StringBuffer("Kotlin")

    println("Length: ${buffer2.length}") // "Length: 6"
    println("Capacity: ${buffer2.capacity()}") // "Capacity: 22" (начальная ёмкость + длина строки = 16 + 6)

    // Указание ёмкости StringBuffer во время создания
    val buffer3 = StringBuffer(10) // начальная ёмкость 10 символов
    ...
    // Время выполнения задачи находим, вычитая время начала из времени окончания.
    var executionTime: Long = System.currentTimeMillis() - time

    // StringBuffer gets it finished in 2256 ms,
    ...
    // while StringBuilder gets it finished in just 159 ms!
